...
COMBAT_TREES = [
    ('General', 'GC',
     ...
     [
        ...
        ('GC2', 'Second Wind', 0,
         'The first time you drop to 50% HP or below in a combat, you immediately regain FOR + STA HP. No AP cost, no action required — it triggers automatically. Once per day.'),
...
OPERATIONS_TREES = [
    ('General', 'GO',
     ...
     [
        ('GO1', 'Measured Appeal', 0,
         '+1 die on Persuasion checks. When you succeed on a Persuasion check by 5+, the NPCs Disposition shifts +1. They appreciate a good argument when they see it.'),
        ('GO2', 'Calculated Deception', 0,
         '+1 die on Deception checks. When you are caught in a lie, you may immediately attempt a second Deception check (no penalty) to reframe the revelation — on success, what was exposed becomes a partial truth or a misunderstanding rather than a confirmed lie. On failure, the original exposure stands.'),
        ...
        ('GO8', 'Lie Detector', 0,
         '+1 die on Intuition checks made to detect deception. When an NPC lies to you and you succeed on a contested Intuition check, the GM confirms the statement was false. Once per scene, you may ask the GM whether a specific statement an NPC made this scene was entirely truthful.'),
        ('GO9', 'Null Sense', 0,
         'Through accumulated exposure and attentiveness rather than any NL ability, you have learned to notice when something is anomalously wrong. When you enter a space with significant NL activity, or encounter a creature, object, or phenomenon with an active Imprint, the GM gives you a vague signal without prompting. On a successful Survival or Null Theory check, you can narrow down which Continuity Constant is being violated.'),
        ('GO10', 'Trader', 0,
         'When buying, selling, or bartering, you negotiate better terms than face value. On a successful Persuasion or Intuition check, you either pay 20% less for a purchase, receive 20% more value when selling, or extract one additional concession from a trade already agreed upon.'),
    ]),
    ('The Face', 'TF',
    ...
    ('Animal Handler', 'AH',
     ...
     [
        ...
        ('AH2', 'Working Bond', 2,
         'When your companion is present during a social interaction, its presence can shift the dynamic. Calm, well-behaved animals make some NPCs more at ease — the GM may grant +1 die on relevant social checks. Conversely, an imposing or unsettling companion may impose −1 die. Additionally, you can attempt an Animal Handling check to read the emotional state of any animal you can observe.'),
        ('AH3', 'Follow the Trail', 3,
         'You can track a target across significant distances using your companion as the primary tracker. Roll Animal Handling. On success, you can follow a trail up to PRE × 24 hours old. On a strong success (beat DC by 5+), you also learn one piece of general information about the target.'),
    ]),
    ('Navigator', 'NV',
     ...
     [
        ...
        ('NV2', 'Live off the Land', 2,
         'Once per day of travel, you may attempt to forage food and water from the environment — roll Survival vs a DC set by the region\'s density and hostility. On success, you produce sufficient rations for a single party member, for every +5 above the DC, you generate extra rations for one more party member. Additionally, when a Journey Event would reduce the party\'s ration supply, you may make a Null Navigation or Survival check to reduce the loss by half.'),
        ('NV3', 'Safe Passage', 3,
    ...
    ('Sleight of Hand', 'SH',
     ...
     [
        ('SH1', 'Quick Hands', 1,
         '+1 die on Sleight of Hand checks. When you fail a Sleight of Hand check, you may immediately make a second check — Stealth or Deception (your choice) — to avoid being identified as the one responsible. On success, the failure is noticed but not traced to you.'),
        ('SH2', 'Locksmith', 2,
    ...
    ('Field Medic', 'FM',
     ...
     [
        ...
        ('FM2', 'Sustained Care', 2,
         'During a Long Rest, choose a number of allies up to your INS. Those allies recover 75% of their Max HP instead of 50%, and their Fatigue reduces by 2 levels instead of 1. Additionally, you may attempt to treat one of these allies who has a Critical Injury — roll Medicine vs DC 18.'),
        ('FM3', 'Deep Recovery', 3,
